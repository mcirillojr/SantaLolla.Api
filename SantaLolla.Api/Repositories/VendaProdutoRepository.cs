...
namespace SantaLolla.Api.Repositories
{
    public class VendaProdutoRepository : IVendaProdutoRepository
    {
        ...
        public VendaProdutoRepository(SqlConnectionFactory connectionFactory)
        {
            _connectionFactory = connectionFactory;
        }

        public async Task<IEnumerable<VendaProdutoResponse>> ListarAsync(
        ...
        )
        {
            ...
            var apelido = PrepararFiltroLike(filtro.Apelido);
            var cliente = PrepararFiltroLike(filtro.Cliente);
            ...
            const string sql = @"
                ...
                (
                    ...
                    WHERE 1 = 1
                      ...
                      AND (@Venda IS NULL OR CODIGO_VENDA = @Venda)
                      AND (@CodVendedor IS NULL OR CODVENDEDOR = @CodVendedor)
                      AND (@Referencia IS NULL OR REFERENCIA LIKE @Referencia)
                      AND (@Apelido IS NULL OR APELIDO LIKE @Apelido)
                      AND (@Cliente IS NULL OR CLIENTE LIKE @Cliente)
                      AND (@DataVendaInicio IS NULL OR DATA_VENDA >= @DataVendaInicio)
                      AND (@DataVendaFim IS NULL OR DATA_VENDA <= @DataVendaFim)
            ...
            var linhas = await connection.QueryAsync<VendaProdutoLinha>(
                sql,
                new
                {
                    Rede = NormalizarTexto(filtro.Rede),
                    Venda = NormalizarTexto(filtro.Venda),
                    CodVendedor = NormalizarTexto(filtro.CodVendedor),
            ...
            var retorno = linhas
                ...
                .Select(g =>
                {
                    ...
                    return new VendaProdutoResponse
                    {
                        ...
                        LastUpdateOrigem = venda.LastUpdateOrigem,
                        Cliente = venda.Cliente,
                        Vendedor = venda.Vendedor,
                        Condicoes = venda.Condicoes,
                        VendaImportada = venda.VendaImportada,
                        Status = venda.Status,
                        Produtos = g.Select(p => new VendaProdutoItemResponse
                        {
